    ...
    ***************************************************************************** */
    ...
    var computedDecorator = createPropDecorator(false, function(
        ...
        asObservableObject(instance).addComputedProp(
            instance,
            propertyName,
            __assign({ get: get, set: set, context: instance }, options)
        )
    })
    ...
    function checkIfStateReadsAreAllowed(observable) {
        if (
            ...
            console.warn(
                "[mobx] Observable " + observable.name + " being read outside a reactive context"
            )
        }
    }
    /**
    ...
    var curMicrotaskReactionCount = 0
        ...
        if (allReactions.length > 0) {
            if (curMicrotaskReactionCount === 0) {
                Promise.resolve().then(function() {
                    if (curMicrotaskReactionCount > 10) {
                        console.warn(
                            "Warning, a sigle microtask loop had " +
    ...
    function flow(generator) {
        if (arguments.length !== 1)
            fail(
                !!process.env.NODE_ENV && "Flow expects 1 argument and cannot be used as decorator"
            )
    ...
    function whenPromise(predicate, opts) {
        ...
        var res = new Promise(function(resolve, reject) {
            var disposer = _when(
                predicate,
                resolve,
                __assign(__assign({}, opts), { onError: reject })
            )
            cancel = function() {
                disposer()
