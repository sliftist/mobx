        ...
        if (curMicrotaskReactionCount === 0) {
            Promise.resolve().then(() => {
                if (curMicrotaskReactionCount > 10) {
                    console.warn(
                        `Warning, a sigle microtask loop had ${curMicrotaskReactionCount} reactions (which are caused by changes to observables). Batch these actions with @action or runInAction to gain up to a ${curMicrotaskReactionCount}X performance boost!`
                    )
                }
